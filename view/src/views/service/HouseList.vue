<template>
  <div class="house-management-container">
    ...
    <div class="house-list">
      <div
        v-for="item in apiResult.data"
        :key="item.id"
        class="house-card"
        :class="{
          'status-available': item.status === 1,
          ...
          <div class="info-footer">
            ...
            <div class="action-buttons">
              <div @click="houseStatusDeal(item.id)">
                <i
                  :class="item.status === 1 ? 'el-icon-bottom' : 'el-icon-top'"
                ></i>
    ...
    >
      <div class="delete-dialog-content">
        ...
        <p>
          确定删除房源
          <span class="highlight">{{ deletedItem.name }}</span> 吗？
        </p>
      </div>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="confirmDeleted" :loading="deleting"
          >确定</el-button
        >
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    handleChange(val) {
      this.houseQueryDto.status =
        val.value === "null" ? null : Number(val.value);
      this.fetchFreshData();
    },
    listener(text) {
